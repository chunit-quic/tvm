...
def get_span_from_parameter(expr, name_sep="."):
...
class EyeLike(OnnxOpConverter):
    ...
    def _impl_v9(cls, inputs, attr, params):
        ...
        node_span = get_span_from_parameter(inputs[0])
        ...
        in_shape = set_span(_op.shape_of(inputs[0]), node_span)
        zeros = _op.zeros(in_shape, dtype)

        dim = set_span(_op.take(in_shape, _op.const(0)), node_span)
...
class Loop(OnnxOpConverter):
    ...
    def _impl_v11(cls, inputs, attr, params):
        ...
        body_span = get_span(body, subgraph_scope._op_type_dict)
        # set span to inputs of loop body
        for i, v in enumerate(loop_vars):
            loop_vars[i] = set_span(v, make_parameter_span([v.name_hint, body_span]))
...
class GraphProto:
    ...
    def _construct_nodes(self, graph):
        ...
        for node in graph.node:
            ...
            node_span = get_span(node, self._op_type_dict)
            self._set_parameter_span(node, node_span)
    ...
    def _set_parameter_span(self, node, node_span):
        for i in node.input:
            if i != "":
                ...
                if isinstance(expr, (relay.Var, relay.Constant)):
                    expr_with_span = set_span(expr, make_parameter_span([node_span, name]))
